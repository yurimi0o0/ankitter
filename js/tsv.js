...
export function rowsToCards(sourceId, rows, mapping, options = {}) {
  ...
  return rows.map((row, rowIndex) => {
    ...
    const tags = tagCols
      ...
      .map((t) => t.trim())
      .filter(Boolean);
    const mediaFields = options.mediaEnabled === false
      ? []
      : buildMediaFields(row, mapping, {
        questionCol,
        answerCol,
        mediaColumns: options.mediaColumns,
      });
    return {
...
function buildMediaFields(row, mapping, { questionCol, answerCol, mediaColumns }) {
  return row
    .map((cell, colIndex) => {
      const raw = (cell || '').trim();
